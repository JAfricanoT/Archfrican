...
**Virtualization (opt-in, on-demand):** pick "Máquinas virtuales" in `archfrican-defaults` (same
"pick it, it installs" flow as remote-access apps) to install KVM/QEMU + libvirt + virt-manager —
the Linux-native, hardware-accelerated hypervisor. Wired straight into nftables (this repo's only
firewall, no iptables anywhere) via libvirt's own nftables backend.
