...
run_phase2() {                # run_phase2 [single-module]
  ...
  if ! chezmoi init --apply --force --source "$REPO_ROOT/home"; then
    ...
    chezmoi init --apply --force --source "$REPO_ROOT/home" \
      || die "chezmoi failed — packages installed but dotfiles NOT deployed. Clear it (pkill -x chezmoi; rm -f ~/.config/chezmoi/chezmoistate.boltdb*) then re-run: chezmoi init --apply --force --source $REPO_ROOT/home"
  fi
  current_module=""
